#!/usr/bin/env python3
"""
Train a graph attention network with an MLP head to classify disulfide-bonded cysteines.
...
and trains a GAT-based node classifier that leverages edge distance features.
"""
...
def graph_entry_to_data(entry: dict) -> Data:
    ...
    edge_attrs = entry.get("edge_attrs", [])
    ...
    if edges:
        ...
        if edge_attrs:
            ...
            edge_attr = torch.zeros((len(edges), 2), dtype=torch.float)
    else:
